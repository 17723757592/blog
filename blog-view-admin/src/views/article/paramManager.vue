<template>
	<section>
		...
		<el-table border :data="tableDatas" v-loading="listLoading" @selection-change="handleSelectionChange"
			...
			<el-table-column prop="paramKey" label="参数键" width="160" fixed="left">
			</el-table-column>
			<el-table-column prop="paramDesc" label="参数描述" width="260" fixed="left">
			</el-table-column>
			<el-table-column prop="paramValue" label="参数值" width="430" fixed="left">
			</el-table-column>
		...
		<!--新增界面-->
		<el-dialog title="新增参数" :visible.sync="addFormVisible"  :close-on-click-modal="false">
			<el-form :model="addForm" label-width="80px"  ref="addForm">
				<el-form-item label="参数键" prop="paramKey">
					<el-input v-model="addForm.paramKey" auto-complete="off"></el-input>
				</el-form-item>
				<el-form-item label="参数描述" prop="paramDesc">
					<el-input v-model="addForm.paramDesc" auto-complete="off"></el-input>
				</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			//提交新增表单数据
            addSubmit(){
                this.$http.post("/blogParam/save", this.addForm).then(result=>{
			...
			delBatch(){
				this.$confirm('确定删除这些数据吗？', '请确认', {
					...
					console.log(param)
					this.$http.post("/blogParam/deleteBatch", param).then(result=>{
						if(result.data.success){
							this.$message({
								type: 'success',
								message: result.data.message
			...
			del(row){
				this.$confirm('确定删除此条数据吗？', '请确认', {
					...
					this.$http.delete("/blogParam/" + row.userId).then(result=>{
						if(result.data.success){
							this.$message({
								type: 'success',
								message: result.data.message
            ...
            getTableData(){
				...
				//分页查询
                this.$http.post("/blogParam/pagelist", para).then(result=>{
