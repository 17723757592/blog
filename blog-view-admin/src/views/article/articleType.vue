<template>
	<section>
		...
		<!--新增界面-->
		<el-dialog title="新增类型" :visible.sync="addFormVisible"  :close-on-click-modal="false">
...
<script>
	export default {
		...
		methods: {
			...
			//提交新增表单数据
            addSubmit(){
                this.$http.post("/blogArticleType/save", this.addForm).then(result=>{
			...
			delBatch(){
				this.$confirm('确定删除这些数据吗？', '请确认', {
					...
					console.log(param)
					this.$http.post("/blogArticleType/deleteBatch", param).then(result=>{
						if(result.data.success){
							this.$message({
								type: 'success',
								message: result.data.message
			...
			del(row){
				this.$confirm('确定删除此条数据吗？', '请确认', {
					...
					this.$http.delete("/blogArticleType/" + row.userId).then(result=>{
						if(result.data.success){
							this.$message({
								type: 'success',
								message: result.data.message
            ...
            getTableData(){
				...
				//分页查询
                this.$http.post("/blogArticleType/pagelist", para).then(result=>{
